    ...
    const draw = () => {
      ...
      for (let i=0;i<pts.length;i++) for (let j=i+1;j<pts.length;j++) {
        ...
        if (d < 100) {
          ...
          ctx.strokeStyle=`rgba(90,160,255,${.09*(1-d/100)})`; ctx.lineWidth=.5; ctx.stroke()
        }
      }
...
function BinanceChart({ data }) {
  ...
  useEffect(() => {
    ...
    ;["ma99","ma25","ma7"].forEach(key => {
      ctx.strokeStyle=MA_COLORS[key]; ctx.lineWidth=1.2; ctx.beginPath(); let s=false
      data.forEach((d,i)=>{ if(d[key]==null)return; const x=toX(i),y=toY(d[key]); if(!s){ctx.moveTo(x,y);s=true}else ctx.lineTo(x,y) })
      ctx.stroke()
    })
    data.forEach((d,i)=>{
      const x=toX(i), up=d.close>=d.open, col=up?"#26a69a":"#ef5350"
      const bodyTop=toY(Math.max(d.open,d.close)), bodyBot=toY(Math.min(d.open,d.close))
      const bodyH=Math.max(1,bodyBot-bodyTop), hw=Math.max(.5,barW/2)
      ctx.strokeStyle=col; ctx.lineWidth=1
      ctx.beginPath(); ctx.moveTo(x,toY(d.high)); ctx.lineTo(x,bodyTop); ctx.moveTo(x,bodyBot); ctx.lineTo(x,toY(d.low)); ctx.stroke()
      ctx.fillStyle=up?"rgba(38,166,154,0.9)":"rgba(239,83,80,0.85)"; ctx.fillRect(x-hw,bodyTop,barW,bodyH)
    })
    data.forEach((d,i)=>{
      const x=toX(i),hw=Math.max(.5,barW/2),vy=toVY(d.volume)
    ...
    if (!isMobile) {
      ...
      ;[["MA(7)","ma7"],["MA(25)","ma25"],["MA(99)","ma99"]].forEach(([label,key])=>{
        const last=[...data].reverse().find(d=>d[key]!=null); if(!last)return
    ...
    <div style={{position:"relative"}}>
      {hd && (
        <div style={{position:"absolute",top:18,left:PAD.left+4,display:"flex",gap:10,zIndex:2,pointerEvents:"none",flexWrap:"wrap"}}>
          {!isMobile && <span style={{fontFamily:"DM Mono,monospace",fontSize:10,color:"rgba(140,180,255,0.5)"}}>{new Date(hd.time).toLocaleDateString([],{year:"numeric",month:"short",day:"numeric"})}</span>}
...
function Calculator({ offers, side, fiat: globalFiat, crypto: globalCrypto }) {
  ...
  const validOffers = num > 0 ? offers.filter(o=>num>=o.min_amount&&num<=o.max_amount) : offers
  const outOfRange = num > 0 && validOffers.length === 0

  const best = validOffers.length
  ...
  const effPrice = best ? (side==="BUY"?best.price*(1+(best.commission||0)/100):best.price*(1-(best.commission||0)/100)) : null
  const result = effPrice && num ? (side==="BUY"?num/effPrice:num*effPrice) : null
  ...
  return (
    <div className="glass-strong" style={{padding:"15px 16px",marginBottom:13}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:120,display:"flex",gap:0}}>
        ...
        <div style={{flex:1,minWidth:120,background:"rgba(4,10,28,0.6)",border:`1px solid ${outOfRange?"rgba(239,83,80,0.3)":"rgba(80,130,255,0.1)"}`,borderRadius:10,padding:"10px 10px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
      ...
      {outOfRange && num>0 && (
        <div style={{marginTop:8,fontFamily:"DM Mono,monospace",fontSize:11,color:"#ef5350"}}>
          ...
          {offers.length>0&&` — available range: ${offers[0].min_amount.toLocaleString()}–${Math.max(...offers.map(o=>o.max_amount)).toLocaleString()}`}
        </div>
      )}
      {best && result && !outOfRange && (
        <div style={{marginTop:8,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          <span style={{fontFamily:"DM Mono,monospace",fontSize:10,color:"rgba(100,150,255,0.4)"}}>
            @ {best.price.toFixed(3)}{best.commission>0&&<span style={{color:"rgba(239,83,80,0.6)"}}> +{best.commission}% → {effPrice?.toFixed(3)}</span>}
          </span>
          <span style={{fontFamily:"DM Mono,monospace",fontSize:10,color:"rgba(38,166,154,0.5)"}}>
            {best.min_amount.toLocaleString()}–{best.max_amount.toLocaleString()}
          </span>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{width:"100%",minWidth:0}}>
      <div className="glass-strong" style={{marginBottom:13,overflow:"hidden"}}>
        {/* Symbol + interval — horizontally scrollable */}
          ...
          <div style={{display:"flex",borderBottom:"1px solid rgba(50,90,180,0.08)",minWidth:"max-content",padding:"10px 14px 0"}}>
            {CHART_SYMBOLS.map(s=>(
              <button key={s.id} onClick={()=>setSymbol(s.id)} style={{
                padding:"6px 11px",border:"none",cursor:"pointer",fontFamily:"DM Mono,monospace",fontSize:12,fontWeight:600,transition:"all .2s",whiteSpace:"nowrap",
            ...
            <div style={{marginLeft:"auto",display:"flex",gap:2,alignItems:"center",paddingBottom:8,paddingLeft:10}}>
              {INTERVALS.map(iv=>(
                <button key={iv} onClick={()=>setInterval(iv)} style={{
                  padding:"4px 8px",borderRadius:5,border:"none",cursor:"pointer",fontFamily:"DM Mono,monospace",fontSize:11,transition:"all .2s",whiteSpace:"nowrap",
        ...
        <div style={{padding:"12px 16px 4px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          ...
          {last && <>
            <span style={{fontSize:window.innerWidth<640?22:28,fontWeight:800,color:"#fff",letterSpacing:"-0.02em",fontFamily:"DM Mono,monospace"}}>
      ...
      <div className="glass-strong" style={{overflow:"hidden"}}>
        ...
        {loadingTrending
          ...
          : <>
            <div style={{padding:"8px 16px",display:"grid",gridTemplateColumns:"1fr 1fr 0.7fr 0.9fr",gap:8,borderBottom:"1px solid rgba(50,90,180,0.05)"}}>
            ...
            {trending.map((t,i)=>(
              <a key={i} href={`https://www.binance.com/en/trade/${t.symbol}_USDT`} target="_blank" rel="noreferrer"
                style={{padding:"8px 16px",display:"grid",gridTemplateColumns:"1fr 1fr 0.7fr 0.9fr",gap:8,alignItems:"center",borderBottom:"1px solid rgba(50,90,180,0.03)",textDecoration:"none",opacity:0,transform:"translateY(5px)",animation:`fadeUp .3s forwards ${i*18}ms`,transition:"background .15s"}}
                onMouseEnter={e=>e.currentTarget.style.background="rgba(80,130,255,0.04)"}
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <div style={{minWidth:0}}>
                  <div style={{fontFamily:"DM Mono,monospace",fontSize:12,fontWeight:600,color:"#cce0ff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.symbol}<span style={{color:"rgba(100,150,255,0.3)",fontWeight:400}}>/USDT</span></div>
                  <div style={{fontFamily:"DM Mono,monospace",fontSize:9,color:"rgba(80,130,255,0.35)",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{COIN_NAMES[t.symbol]||""}</div>
                </div>
                ...
                <span style={{fontFamily:"DM Mono,monospace",fontSize:10,color:"rgba(80,130,255,0.4)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
  ...
  const loadOffers = useCallback((f,c,s,ex,sr,mr)=>{
    ...
    fetch(`http://localhost:8000/p2p?fiat=${f}&crypto=${c}&side=${s}&exchange=${ex}&sort=${sr}&min_rate=${mr}`)
      ...
      .then(d=>{
        ...
        const newPrices={}; newOffers.forEach(o=>{ newPrices[o.advertiser]=o.price })
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .glass{background:rgba(6,14,36,0.5);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1.5px solid rgba(70,120,220,0.14);border-radius:16px;}
        ...
        @media(max-width:640px){
          .wrapper{padding:16px 11px 60px;}
          .bg-orb{display:none;}
          .glass-strong,.glass{border-radius:13px;}
          ...
          .table-header{grid-template-columns:1.1fr 0 1.4fr 0.45fr 1.7fr !important;padding:8px 10px;gap:5px;}
          .table-header .th:nth-child(2){display:none;}
          .row{grid-template-columns:1.1fr 0 1.4fr 0.45fr 1.7fr !important;padding:8px 10px;gap:5px;}
        ...
        {mode==="market"?<MarketMode/>:(<>
          ...
          <div className="glass-strong" style={{overflow:"hidden"}}>
            {loading&&offers.length===0
              ...
              :offers.length===0?<div className="loading-state">no offers found</div>
              :<>
